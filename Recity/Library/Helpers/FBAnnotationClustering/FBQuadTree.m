...
- (BOOL)insertAnnotation:(id<RCAnnotationProtocol>)annotation
{
    return [self insertAnnotation:annotation toNode:self.rootNode];
}

- (BOOL)removeAnnotation:(id<RCAnnotationProtocol>)annotation
{
    return [self removeAnnotation:annotation fromNode:self.rootNode];
}

- (BOOL)removeAnnotation:(id<RCAnnotationProtocol>)annotation fromNode:(FBQuadTreeNode *)node
{
    if (!FBBoundingBoxContainsCoordinate(node.boundingBox, [annotation searchCoordinate])) {
        return NO;
    }
...
- (BOOL)insertAnnotation:(id<RCAnnotationProtocol>)annotation toNode:(FBQuadTreeNode *)node
{
    if (!FBBoundingBoxContainsCoordinate(node.boundingBox, [annotation searchCoordinate])) {
        return NO;
    }
...
- (void)enumerateAnnotationsInBox:(FBBoundingBox)box withNode:(FBQuadTreeNode*)node usingBlock:(void (^)(id<MKAnnotation>))block
{
    ...
    for (id<RCAnnotationProtocol> annotation in tempArray) {
        if (FBBoundingBoxContainsCoordinate(box, [annotation searchCoordinate])) {
            block(annotation);
        }
    }
