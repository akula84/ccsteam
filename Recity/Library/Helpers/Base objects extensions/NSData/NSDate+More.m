...
- (NSString *)stringInTimeZone:(NSTimeZone *)timeZone WithFormat: (NSString *) format {
    NSDateFormatter *formatter = [NSDateFormatter new];
	formatter.timeZone = timeZone;
	[formatter setTimeStyle:NSDateFormatterNoStyle];
//    formatter.locale = [NSLocale currentLocale]; // Necessary?
	formatter.locale = [NSLocale localeWithLocaleIdentifier:@"ru_RU_POSIX"];
...
// !     @example @"EEEE dd MMMM" => пятница 28 августа
- (NSString *)localizedDescriptionWithFormat:(NSString *)format {
    NSString *result;
...
#pragma mark - String Properties
- (NSString *) stringInTimeZone:(NSTimeZone *)timeZone format:(NSString *)format {
    NSDateFormatter *formatter = [NSDateFormatter new];
    formatter.timeZone = timeZone;
    [formatter setTimeStyle:NSDateFormatterNoStyle];
    //    formatter.locale = [NSLocale currentLocale]; // Necessary?
...
- (BOOL) isSameMonthAsDate: (NSDate *) aDate {
    NSDateComponents *components1 = [[NSDate currentCalendar] components:(NSCalendarUnit)(NSCalendarUnitYear | NSCalendarUnitMonth) fromDate:self];
    NSDateComponents *components2 = [[NSDate currentCalendar] components:(NSCalendarUnit)(NSCalendarUnitYear | NSCalendarUnitMonth) fromDate:aDate];
...
- (NSDate *) dateByAddingYears: (NSInteger) dYears {
    ...
    NSDate *newDate = [[NSCalendar currentCalendar] dateByAddingComponents:dateComponents toDate:self options:(NSCalendarOptions)0];
    return newDate;
}
...
-(NSDate*) dateWithoutTimeComponentForTimeZone:(NSTimeZone *) timeZone {
	// Deleting time component from NSDate object.
	
	NSCalendar *calendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
	NSDateComponents *components = [calendar components:(NSCalendarUnit)(NSCalendarUnitYear|NSCalendarUnitMonth|NSCalendarUnitDay)
											   fromDate:self];
...
- (NSDateComponents *) componentsWithOffsetFromDate: (NSDate *) aDate {
	NSDateComponents *dTime = [[NSDate currentCalendar] components:componentFlags fromDate:aDate toDate:self options:(NSCalendarOptions)0];
	return dTime;
}
...
- (NSInteger)distanceInDaysToDate:(NSDate *)anotherDate {
    ...
    NSDateComponents *components = [gregorianCalendar components:NSCalendarUnitDay fromDate:self toDate:anotherDate options:(NSCalendarOptions)0];
...
+ (NSString *)hms:(NSInteger)totalSeconds {
...
+ (NSString *)ms:(NSInteger)totalSeconds {
    NSInteger seconds = totalSeconds % 60;
    NSInteger minutes = (totalSeconds / 60) % 60;
